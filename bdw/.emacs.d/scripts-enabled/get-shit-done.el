...
(defun build-repo-url-list (repo-owner repo-list &optional suffix)
  (mapcar (lambda (repo)
	    (cons repo (concat "https://code.aura-software.com/" repo-owner "/" repo  (or suffix "/issues/new") ))
	    )
	  repo-list
	  ))

(setq gitlab-ticket-urls
      (append
       (build-gitlab-ticket-urls "bretweinraub" '("bdw" "fin3" "bash_profiles" "sales"))
       (build-gitlab-ticket-urls
	"aura"
	'("BrightSettings" "alexion" "aura-bright-child-theme" "aur-bright-login-box-plugin" "aura-bright-multisite-search" 
